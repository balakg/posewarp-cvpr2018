...
#if(i == 6):
#	mask_i = makeGaussianMap(img_width,img_height,joints[10,:],17**2,17**2,0)
#	mask[:,:,i] = np.maximum(mask[:,:,i], mask_i/(np.amax(mask_i) + 1e-6))
#if(i == 8):
#	mask_i = makeGaussianMap(img_width,img_height,joints[13,:],17**2,17**2,0)
#	mask[:,:,i] = np.maximum(mask[:,:,i], mask_i/(np.amax(mask_i) + 1e-6))
