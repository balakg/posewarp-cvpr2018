...
def get_example_info(vid_name, frame_num, box, X):
...
def warpExampleGenerator(vid_info_list,param,do_augment=True,return_pose_vectors=False,return_class=False):
	...
	while True:
		...
		for i in xrange(batch_size):
			...
			example0 = get_example_info(vid_path, frames[0], vid_bbox, vid_X)
			example1 = get_example_info(vid_path, frames[1], vid_bbox, vid_X)
			
			I0,joints0,scale0,pos0 = read_example_info(example0)
			I1,joints1,scale1,pos1 = read_example_info(example1)
...
def makeJointHeatmaps(height,width,joints,sigma,pose_dn):
	...
	for i in xrange(n_joints):
		...
		H[:,:,i] = makeGaussianMap(width,height,joints[i,:],var,var,0.0)

	return H

def makeGaussianMap(img_width,img_height,center,var_x,var_y,theta):
